...
import { CustomEntityProperties } from '../model/customEntityProperties';
...
export class CustomEntitiesApi {
     ...
     * @summary [EARLY ACCESS] GetAllCustomEntityProperties: Get all properties related to a Custom Entity instance.
     * @param entityType The type of Custom Entity.
     * @param identifierType An identifier type attached to the Custom Entity instance.
     * @param identifierValue The identifier value.
     * @param identifierScope The identifier scope.
     * @param asAt The AsAt datetime at which to retrieve the Custom Entity properties.
     * @param effectiveAt The effective datetime at which to get the Custom Entity properties. Defaults to the current LUSID system datetime if not specified.
     */
    public async getAllCustomEntityProperties (entityType: string, identifierType: string, identifierValue: string, identifierScope: string, asAt?: Date, effectiveAt?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CustomEntityProperties;  }> {
        const localVarPath = this.basePath + '/api/customentities/{entityType}/{identifierType}/{identifierValue}/properties'
            .replace('{' + 'entityType' + '}', encodeURIComponent(String(entityType)))
            .replace('{' + 'identifierType' + '}', encodeURIComponent(String(identifierType)))
            .replace('{' + 'identifierValue' + '}', encodeURIComponent(String(identifierValue)));
        let localVarQueryParameters: any = {};
        ...
        // verify required parameter 'entityType' is not null or undefined
        if (entityType === null || entityType === undefined) {
            throw new Error('Required parameter entityType was null or undefined when calling getAllCustomEntityProperties.');
        }

        // verify required parameter 'identifierType' is not null or undefined
        if (identifierType === null || identifierType === undefined) {
            throw new Error('Required parameter identifierType was null or undefined when calling getAllCustomEntityProperties.');
        }

        // verify required parameter 'identifierValue' is not null or undefined
        if (identifierValue === null || identifierValue === undefined) {
            throw new Error('Required parameter identifierValue was null or undefined when calling getAllCustomEntityProperties.');
        }

        // verify required parameter 'identifierScope' is not null or undefined
        if (identifierScope === null || identifierScope === undefined) {
            throw new Error('Required parameter identifierScope was null or undefined when calling getAllCustomEntityProperties.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CustomEntityProperties;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "CustomEntityProperties");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
     ...
     * @param propertyKeys A list of property keys from the \&#39;CustomEntity\&#39; domain to decorate onto              the custom entities of any type supported by that property (defined within the property definition CustomEntityTypes).              These must have the format {domain}/{scope}/{code}, for example \&#39;CustomEntity/someScope/id\&#39;.
     */
