...
export class ReferencePortfolioApi {
     ...
     * @summary DeleteReferencePortfolioConstituentAdjustment: Delete constituents adjustment
     ...
     * @param effectiveAt The effective datetime or cut label at which the constituents adjustment was made.
     */
    public async deleteReferencePortfolioConstituentAdjustment (scope: string, code: string, effectiveAt: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DeletedEntityResponse;  }> {
        const localVarPath = this.basePath + '/api/referenceportfolios/{scope}/{code}/constituentsadjustment'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
