...
export class PortfoliosApi {
     ...
     */
    public async upsertPortfolioProperties (scope: string, code: string, portfolioProperties?: { [key: string]: PropertyValue; }, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PortfolioProperties;  }> {
        const localVarPath = this.basePath + '/api/portfolios/{scope}/{code}/properties'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
