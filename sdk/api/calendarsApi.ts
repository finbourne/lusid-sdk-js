...
import { ResolveTenorsRequest } from '../model/resolveTenorsRequest';
import { ResolveTenorsResponse } from '../model/resolveTenorsResponse';
...
export class CalendarsApi {
     ...
     * @summary [EARLY ACCESS] ResolveTenors: Resolve tenor strings to settlement dates.
     * @param resolveTenorsRequest Request containing start date, calendars, spot days, tenors, and optional conventions
     */
    public async resolveTenors (resolveTenorsRequest: ResolveTenorsRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResolveTenorsResponse;  }> {
        const localVarPath = this.basePath + '/api/calendars/tenors/resolve';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        // verify required parameter 'resolveTenorsRequest' is not null or undefined
        if (resolveTenorsRequest === null || resolveTenorsRequest === undefined) {
            throw new Error('Required parameter resolveTenorsRequest was null or undefined when calling resolveTenors.');
        }
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(resolveTenorsRequest, "ResolveTenorsRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ResolveTenorsResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ResolveTenorsResponse");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
