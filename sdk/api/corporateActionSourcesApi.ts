...
export class CorporateActionSourcesApi {
     ...
     * @summary [EARLY ACCESS] DeleteCorporateActionSource: Delete a corporate action source
     ...
     * @param timelineScope The scope of the Timeline, used to override the AsAt, and fetch post close activity data.              If this is provided, timelineCode and closedPeriodId must also be provided.
     * @param timelineCode The code of the Timeline, used to override the AsAt, and fetch post close activity data.              If this is provided, timelineScope and closedPeriodId must also be provided.
     * @param closedPeriodId The code of the ClosedPeriod attached to the timeline, used to override the AsAt, and fetch post close activity data.              If this is provided, timelineScope and timelineCode must also be provided.
     */
    public async getInstrumentEvents (scope: string, code: string, asAt?: Date, limit?: number, page?: string, filter?: string, timelineScope?: string, timelineCode?: string, closedPeriodId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PagedResourceListOfInstrumentEventHolder;  }> {
        const localVarPath = this.basePath + '/api/corporateactionsources/{scope}/{code}/instrumentevents'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'code' + '}', encodeURIComponent(String(code)));
        let localVarQueryParameters: any = {};
