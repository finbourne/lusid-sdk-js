...
import { AccessControlledAction } from '../model/accessControlledAction';
...
export class ApplicationMetadataApi {
     ...
     * @param version The requested version of the Excel plugin
     */
    public async getExcelDownloadUrl (version?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: string;  }> {
        const localVarPath = this.basePath + '/api/metadata/downloads/excel';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        let localVarFormParams: any = {};
     ...
     */
    public async listAccessControlledActions (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<AccessControlledAction>;  }> {
        const localVarPath = this.basePath + '/api/metadata/access/actions';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        let localVarFormParams: any = {};
        ...
        return new Promise<{ response: http.IncomingMessage; body: Array<AccessControlledAction>;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
                    body = ObjectSerializer.deserialize(body, "Array<AccessControlledAction>");
                    if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                        resolve({ response: response, body: body });
                    } else {
                        reject({ response: response, body: body });
