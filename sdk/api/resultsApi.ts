...
export class ResultsApi {
     ...
     */
    public async getResults (scope: string, key: string, date: Date, asAt?: Date, sortBy?: Array<string>, start?: number, limit?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Results;  }> {
     ...
     */
    public async upsertResults (scope: string, key: string, date: Date, request?: CreateResults, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Results;  }> {
        const localVarPath = this.basePath + '/api/results/{scope}/{key}/{date}'
            .replace('{' + 'scope' + '}', encodeURIComponent(String(scope)))
            .replace('{' + 'key' + '}', encodeURIComponent(String(key)))
            .replace('{' + 'date' + '}', encodeURIComponent(String(date)));
