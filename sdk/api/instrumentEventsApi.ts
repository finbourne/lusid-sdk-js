...
import { QueryBucketCashFlowDrillDownRequest } from '../model/queryBucketCashFlowDrillDownRequest';
...
import { ResourceListOfCashFlowDetail } from '../model/resourceListOfCashFlowDetail';
...
export class InstrumentEventsApi {
     ...
     * @summary QueryBucketCashFlowDrillDown: Returns the individual cashflows that make up a single cashflow bucket, with their source lineage.
     * @param queryBucketCashFlowDrillDownRequest The Query Information.
     */
    public async queryBucketCashFlowDrillDown (queryBucketCashFlowDrillDownRequest?: QueryBucketCashFlowDrillDownRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ResourceListOfCashFlowDetail;  }> {
        const localVarPath = this.basePath + '/api/instrumentevents/$queryBucketCashFlowDrillDown';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['text/plain', 'application/json', 'text/json'];
        // give precedence to 'application/json'
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(queryBucketCashFlowDrillDownRequest, "QueryBucketCashFlowDrillDownRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ResourceListOfCashFlowDetail;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ResourceListOfCashFlowDetail");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
